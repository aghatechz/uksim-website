...
export async function POST(req: NextRequest) {
  try {
    ...
    const { fullName, phone, whatsapp, address, city, selectedPackage, selectedPrice, selectedImage, selectedCarrier } = body;

    // Generate order ID (e.g. VOD-849201)
    const rand = Math.floor(100000 + Math.random() * 900000);
    const orderId = `VOD-${rand}`;

    // Determine package image & carrier fallback
    ...
    // Save order to store & file/database
    const createdOrder = addOrder({
      id: orderId,
      customerName: fullName || "Valued Customer",
      phone: phone || whatsapp || "03000000000",
      email: whatsapp ? `${phone}@whatsapp.com` : undefined,
      city: city || "Pakistan",
      address: address || "Standard Courier Delivery Address",
      items: [
        {
          id: `item-${Date.now()}`,
          name: selectedPackage || "Official SIM Card Package",
          qty: 1,
          price: Number(selectedPrice) || 3500,
          image: pkgImage,
          carrier: pkgCarrier,
        },
      ],
      totalAmount: Number(selectedPrice) || 3500,
    ...
    const itemName = selectedPackage || "Official SIM Card Package";
    const itemCarrier = pkgCarrier || "Vodafone UK";
    const formattedPrice = `Rs. ${Number(selectedPrice || 3500).toLocaleString()}`;
    const cleanWaPhone = (whatsapp || phone || "").replace(/\D/g, "").replace(/^0/, "92");

    // Local file path resolution for CID email attachment
    const pathModule = await import("path");
    const fsModule = await import("fs");
    
    let rawImg = pkgImage || "/product pictures/Vodafone_img1_202304.jpg";
    ...
    const emailSubject = `📦 New SIM Order #${orderId} - ${fullName} (${city})`;
    const emailHtml = `
<!DOCTYPE html>
<html>
<head>
  ...
  <div style="max-width:760px;width:100%;margin:32px auto;background-color:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);border:1px solid #e2e8f0;">
    ...
    <div style="padding:36px 44px;">
      ...
      <div style="margin-bottom:36px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
          <tr style="border-bottom:1px solid #f1f5f9;background:#ffffff;">
            ...
            <td style="padding:13px 20px;color:#0f172a;font-weight:600;">${fullName}</td>
          </tr>
          <tr style="border-bottom:1px solid #f1f5f9;background:#f8fafc;">
            <td style="padding:13px 20px;color:#64748b;font-weight:500;">Phone Number</td>
            <td style="padding:13px 20px;color:#0f172a;font-weight:600;font-family:monospace;">${phone}</td>
          </tr>
          <tr style="border-bottom:1px solid #f1f5f9;background:#ffffff;">
            <td style="padding:13px 20px;color:#64748b;font-weight:500;">WhatsApp Number</td>
            <td style="padding:13px 20px;color:#0f172a;font-weight:600;font-family:monospace;">${whatsapp || phone}</td>
          </tr>
          <tr style="border-bottom:1px solid #f1f5f9;background:#ffffff;">
            <td style="padding:13px 20px;color:#64748b;font-weight:500;">Delivery Address</td>
            <td style="padding:13px 20px;color:#0f172a;font-weight:500;line-height:1.5;">${address}</td>
          </tr>
          <tr style="background:#f8fafc;">
            <td style="padding:13px 20px;color:#64748b;font-weight:500;">City / Region</td>
            <td style="padding:13px 20px;color:#0f172a;font-weight:600;">${city}, Pakistan</td>
          </tr>
        </table>
      </div>
      ...
      <div style="text-align:center;margin-top:28px;">
        <a href="https://wa.me/${cleanWaPhone}?text=Hi%20${encodeURIComponent(fullName)},%20your%20Vodafone%20SIM%20order%20%23${orderId}%20details%20have%20been%20received!%20Please%20confirm%20your%20delivery%20address." 
    ...
    if (resendApiKey) {
      try {
        ...
        console.log(`[Resend Email] Successfully delivered order ${orderId} notification to ${adminEmail}`);
      } catch (emailErr) {
        console.error("[Resend Email Error]:", emailErr);
      }
    }
    // Attempt 2: Nodemailer SMTP / Gmail App Password
      ...
      try {
        ...
        console.log(`[Nodemailer SMTP] Successfully delivered order ${orderId} notification to ${adminEmail}`);
      } catch (smtpErr) {
        console.error("[Nodemailer Error]:", smtpErr);
      }
    } else {
      console.log(`[Order API] Order ${orderId} created for ${fullName} (${city}). Admin target email: ${adminEmail}`);
    }
