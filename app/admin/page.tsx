...
function AdminOrdersDashboardContent() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "orders" | "pending" | "dispatched" | "delivered" | "catalog" | "blog" | "whatsapp" | "analytics" | "profile" | "settings">("dashboard");
  ...
  // Admin Blog Posts Management State
  ...
  const [blogForm, setBlogForm] = useState({
    ...
    excerpt: "",
    content: "",
    coverImage: "/reviews/WhatsApp Image 2026-08-01 at 10.00.17 AM.jpeg",
    author: "Vodafone SIM Desk",
    readTime: "5 min read",
    keywords: "Vodafone, TikTok Live, Pakistan",
  });
  ...
  // Web Audio API Ring Chime + AI Voice "New Order Received!" Announcement
  const playNotificationChime = (orderInfo?: { customerName?: string; package?: string }) => {
    try {
      if (!soundEnabled) return;

      // 1. Dual-Tone Bell Ring Chime (C6 -> E6 -> G6 -> C7)
      ...
      if (AudioCtx) {
        ...
        osc2.frequency.exponentialRampToValueAtTime(2093.0, now + 0.35);
      ...
      // 2. AI Voice Announcement: "New Order Received!"
      if (voiceAlertEnabled && typeof window !== "undefined" && "speechSynthesis" in window) {
        window.speechSynthesis.cancel();
        const announcementText = orderInfo?.customerName
          ? `Attention Admin! New Order Received from ${orderInfo.customerName}!`
          : "Attention Admin! New Order Received!";
      ...
      if (data.success && Array.isArray(data.orders)) {
        ...
        if (isPoll && seenOrderIdsRef.size > 0) {
          ...
          if (freshOrders.length > 0) {
            if (soundEnabled) playNotificationChime({ customerName: freshOrders[0].customerName, package: freshOrders[0].items?.[0]?.name });
    ...
    <div className={`min-h-screen p-2 sm:p-3 lg:p-4 font-sans flex justify-center items-start transition-colors duration-300 ${isDarkMode ? "bg-[#0B0F17] text-slate-100" : "bg-[#EEF2F6] text-slate-900"
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-1.5 font-sans pt-1">
            ...
            >
              ...
              <div className={`flex items-center ${sidebarCollapsed ? "justify-center" : "gap-3"}`}>
                <Layers className={`w-4 h-4 ${activeTab === "catalog" ? "text-[#E60000]" : "text-slate-400"}`} />
            ...
            <button
              ...
              title="Blog Upload & Articles"
              className={`w-full relative flex items-center ${sidebarCollapsed ? "justify-center px-2 py-3" : "justify-between px-3.5 py-2.5"} rounded-2xl text-xs transition-all cursor-pointer ${activeTab === "blog"
                ? isDarkMode
                  ? "font-extrabold text-white bg-red-950/60 border border-red-800/60"
                  : "font-extrabold text-slate-900 bg-red-50/80 border border-red-200/60"
                : isDarkMode
            ...
            >
              ...
              <div className={`flex items-center ${sidebarCollapsed ? "justify-center" : "gap-3"}`}>
                <FileText className={`w-4 h-4 ${activeTab === "blog" ? "text-[#E60000]" : "text-slate-400"}`} />
                {!sidebarCollapsed && <span>Blog Articles</span>}
              </div>
              {!sidebarCollapsed && (
                <span className="text-[9px] font-black px-2 py-0.5 rounded-md bg-red-100 text-[#E60000]">
                  {blogsList.length}
                </span>
              )}
            </button>
      ...
      <main className="flex-1 space-y-4 font-sans min-w-0">
            ...
            <div className="relative">
              ...
              {showNotificationsModal && (
                <div className="absolute right-0 top-12 w-80 sm:w-96 bg-white border border-slate-200/90 rounded-3xl shadow-2xl z-50 overflow-hidden font-sans animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="p-4 bg-gradient-to-r from-[#E60000] to-[#CC0000] text-white flex items-center justify-between shadow-xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        title={soundEnabled ? "Sound Ring Chime Enabled" : "Sound Muted"}
                        className={`p-1.5 rounded-xl transition-all flex items-center gap-1 text-[10px] font-bold ${soundEnabled ? "bg-white/20 text-white hover:bg-white/30" : "bg-black/20 text-white/60"
                          }`}
                      >
                      ...
                      {/* AI Voice "New Order Received" Toggle */}
                      <button
                        onClick={() => setVoiceAlertEnabled(!voiceAlertEnabled)}
                        title={voiceAlertEnabled ? "AI Voice Alert Enabled ('New Order Received!')" : "Voice Alert Muted"}
                        className={`px-2 py-1 rounded-xl transition-all flex items-center gap-1 text-[10px] font-extrabold border ${voiceAlertEnabled ? "bg-white text-[#E60000] border-white shadow-xs" : "bg-black/20 text-white/60 border-transparent"
                          }`}
                      >
                        <Volume2 className="w-3 h-3" />
                        <span>Voice {voiceAlertEnabled ? "ON" : "OFF"}</span>
                      </button>
                  ...
                  {/* Drawer Controls Bar with Test Voice Button */}
                  <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200/80 flex items-center justify-between text-[11px] font-extrabold">
                    <button
                      onClick={() => playNotificationChime({ customerName: "Muhammad Ali (Lahore)" })}
                      className="bg-red-100 hover:bg-red-200 text-[#E60000] px-2.5 py-1 rounded-full flex items-center gap-1 cursor-pointer transition-all border border-red-200 shadow-2xs"
                      title="Click to test the 'New Order Received!' Voice and Chime"
                    >
                      <Volume2 className="w-3 h-3 text-[#E60000]" />
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="text-[#E60000] hover:text-[#CC0000] flex items-center gap-1 cursor-pointer transition-colors"
                      >
        ...
        <div className={`border p-5 sm:p-6 rounded-[32px] shadow-xs space-y-5 transition-colors duration-300 ${isDarkMode ? "bg-[#111827] border-slate-800 text-white" : "bg-[#F8FAFC] border-slate-200/80 text-slate-900"
                ...
                <div className={`lg:col-span-3 p-5 sm:p-6 rounded-3xl border shadow-2xs flex flex-col justify-between h-[260px] font-sans transition-colors duration-300 ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900"
                  ...
                  <div className="space-y-1">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`text-[10px] font-extrabold px-2.5 py-1 rounded-full border shrink-0 ${Math.round((orders.length / 10) * 100) >= 100
          ...
          {/* VIEW BLOG: ADMIN BLOG MANAGEMENT & UPLOAD */}
          {activeTab === "blog" && (
            <div className="space-y-6 font-sans animate-in fade-in duration-300">
              {/* 1. Top Header Card */}
              <div className={`p-6 sm:p-8 rounded-3xl border shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors duration-300 ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900"}`}>
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <div className="w-10 h-10 rounded-2xl bg-[#E60000] text-white flex items-center justify-center font-black shadow-md shadow-red-600/30">
                      <FileText className="w-5 h-5 text-white" />
                    </div>
                    <div>
                      <h2 className="text-xl sm:text-2xl font-black tracking-tight flex items-center gap-2">
                        Blog Articles &amp; SEO Content Manager
                      </h2>
                      <span className="text-[10px] font-extrabold uppercase text-[#E60000] tracking-wider block">
                        LIVE CONTENT HUB • {blogsList.length} ARTICLES PUBLISHED
                      </span>
                    </div>
                  </div>
                  <p className="text-xs text-slate-400 font-medium mt-2 max-w-2xl leading-relaxed">
                    Create, edit, and publish SEO guides, setup tutorials, and carrier news. Content is instantly indexed and rendered at <span className="font-mono text-slate-300">uksim.pk/blog</span>.
                  </p>
                </div>

                <button
                  onClick={() => setShowBlogModal(true)}
                  className="bg-[#E60000] hover:bg-[#CC0000] text-white text-xs font-black px-6 py-3.5 rounded-full transition-all shadow-md shadow-red-600/20 uppercase tracking-wider flex items-center gap-2 shrink-0 cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
                >
                  <Plus className="w-4 h-4" />
                  Upload New Article
                </button>
              ...
              {/* 2. Top Metrics Grid (4 Cards matching Catalog theme) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {/* Metric 1 */}
                <div className={`p-5 rounded-3xl border shadow-2xs flex flex-col justify-between h-[130px] ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900"}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-slate-400">Total Published Articles</span>
                    <div className="w-8 h-8 rounded-xl bg-red-50 dark:bg-slate-800 text-[#E60000] flex items-center justify-center border border-red-100 dark:border-slate-700">
                      <FileText className="w-4 h-4 text-[#E60000]" />
                    </div>
                  </div>
                  <div>
                    <div className="text-2xl font-black text-[#E60000]">{blogsList.length}</div>
                    <span className="text-[10px] font-bold text-slate-400 mt-0.5 block">Live Articles on Website</span>
                  </div>
                </div>
                ...
                <div className={`p-5 rounded-3xl border shadow-2xs flex flex-col justify-between h-[130px] ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900"}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-slate-400">TikTok Live Guides</span>
                    <div className="w-8 h-8 rounded-xl bg-purple-50 dark:bg-slate-800 text-purple-600 flex items-center justify-center border border-purple-100 dark:border-slate-700">
                      <Flame className="w-4 h-4 text-purple-600" />
                    </div>
                  </div>
                  <div>
                    <div className="text-2xl font-black text-purple-600 dark:text-purple-400">
                      {blogsList.filter((b) => b.category === "TikTok Live").length}
                    </div>
                    <span className="text-[10px] font-bold text-slate-400 mt-0.5 block">Monetization &amp; Streaming</span>
                  </div>
                </div>
                ...
                <div className={`p-5 rounded-3xl border shadow-2xs flex flex-col justify-between h-[130px] ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900"}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-slate-400">UK Banking Guides</span>
                    <div className="w-8 h-8 rounded-xl bg-emerald-50 dark:bg-slate-800 text-emerald-600 flex items-center justify-center border border-emerald-100 dark:border-slate-700">
                      <CreditCard className="w-4 h-4 text-emerald-600" />
                    </div>
                  </div>
                  <div>
                    <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400">
                      {blogsList.filter((b) => b.category === "UK Banking").length}
                    </div>
                    <span className="text-[10px] font-bold text-slate-400 mt-0.5 block">OTP, Wise &amp; Monzo Tutorials</span>
                  </div>
                </div>
                ...
                <div className={`p-5 rounded-3xl border shadow-2xs flex flex-col justify-between h-[130px] ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900"}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[10.5px] font-extrabold uppercase tracking-widest text-slate-400">US Verification</span>
                    <div className="w-8 h-8 rounded-xl bg-pink-50 dark:bg-slate-800 text-pink-600 flex items-center justify-center border border-pink-100 dark:border-slate-700">
                      <Globe className="w-4 h-4 text-pink-600" />
                    </div>
                  </div>
                  <div>
                    <div className="text-2xl font-black text-pink-600 dark:text-pink-400">
                      {blogsList.filter((b) => b.category === "US Verification").length}
                    </div>
                    <span className="text-[10px] font-bold text-slate-400 mt-0.5 block">T-Mobile USA &amp; US Numbers</span>
                  </div>
                </div>
              </div>

              {blogsList.length === 0 ? (
                <div className={`p-12 text-center rounded-3xl border ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white" : "bg-white border-slate-200/80 text-slate-900 shadow-2xs"}`}>
                  <FileText className="w-12 h-12 text-slate-400 mx-auto mb-3" />
                  <h3 className="text-base font-bold mb-1">No Blog Articles Published Yet</h3>
                  <p className="text-xs text-slate-400 mb-4">Click below to publish your first article to the website.</p>
                  <button
                    onClick={() => setShowBlogModal(true)}
                    className="bg-[#E60000] text-white text-xs font-bold px-6 py-2.5 rounded-full cursor-pointer hover:bg-[#CC0000] transition-all"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {blogsList.map((b) => (
                    <div
                      key={b.id || b.slug}
                      className={`group rounded-3xl border overflow-hidden p-5 flex flex-col justify-between transition-all duration-300 shadow-2xs hover:shadow-xl hover:-translate-y-1 ${isDarkMode ? "bg-[#1F2937] border-slate-800 text-white hover:border-slate-700" : "bg-white border-slate-200/80 text-slate-900 hover:border-slate-300"}`}
                    >
                      <div>
                        {/* Image Container */}
                        <div className="relative aspect-video rounded-2xl overflow-hidden bg-slate-100 mb-4 border border-slate-100">
                          <Image
                            src={b.coverImage || "/reviews/WhatsApp Image 2026-08-01 at 10.00.17 AM.jpeg"}
                            alt={b.title}
                            fill
                            className="object-cover group-hover:scale-105 transition-transform duration-500"
                          />
                          <span className="absolute top-3 left-3 bg-[#E60000] text-white text-[10px] font-black uppercase px-3 py-1 rounded-full shadow-sm tracking-wider">
                            {b.category || "SIM Guide"}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-400 font-semibold mb-2">
                          ...
                          <span>⏱️ {b.readTime || "5 min read"}</span>
                        </div>

                        {/* Title */}
                        <h3 className="text-base font-extrabold line-clamp-2 mb-2 leading-snug tracking-tight group-hover:text-[#E60000] transition-colors">
                          {b.title}
                        </h3>
                        ...
                        {b.keywords && b.keywords.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mb-4">
                            {b.keywords.slice(0, 3).map((kw: string, kIdx: number) => (
                              <span key={kIdx} className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-bold px-2 py-0.5 rounded-md">
                                #{kw}
                              </span>
                            ))}
                          </div>
                      ...
                      {/* Card Footer Actions matching Catalog */}
                      <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
                        <Link
                          href={`/blog/${b.slug}`}
                          target="_blank"
                          className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-900 dark:text-[#E60000] text-xs font-extrabold px-4 py-2 rounded-xl transition-all flex items-center gap-1.5 border border-slate-200 dark:border-slate-700"
                        >
                        ...
                        <button
                          ...
                          className="bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 text-xs font-extrabold px-3.5 py-2 rounded-xl transition-all flex items-center gap-1.5 border border-rose-200 dark:border-rose-900 cursor-pointer"
                        >
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4 font-sans">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div
                    className={`relative border-2 border-dashed rounded-2xl p-4 text-center cursor-pointer transition-all hover:border-[#E60000] ${isDarkMode ? "bg-slate-800 border-slate-600 hover:bg-slate-700" : "bg-slate-50 border-slate-300 hover:bg-red-50/30"
      ...
      {/* ================= UPLOAD NEW BLOG ARTICLE MODAL ================= */}
      {showBlogModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4 transition-all duration-300 animate-in fade-in overflow-y-auto">
          <div className={`max-w-2xl w-full rounded-[32px] p-6 sm:p-8 border space-y-6 shadow-2xl my-8 animate-in zoom-in-95 duration-200 ${isDarkMode ? "bg-[#111827] border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}>
            <div className="flex items-center justify-between border-b pb-4 border-slate-200/40">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black tracking-tight">Upload New Blog Article</h3>
                  <p className="text-xs text-slate-400 font-medium">Publish an article to the public blog page</p>
                </div>
              </div>
              <button
                onClick={() => setShowBlogModal(false)}
                className="text-slate-400 hover:text-white p-2 rounded-xl border border-transparent hover:border-slate-700 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveBlog} className="space-y-4 text-xs font-sans">
              <div>
                <label className="block text-[11px] font-extrabold uppercase tracking-wider mb-1.5 text-slate-400">
                  Article Title
                </label>
                <input
                  type="text"
                  required
                  placeholder="e.g. How to Enable TikTok Live in Pakistan in 2026..."
                  value={blogForm.title}
                  onChange={(e) => setBlogForm({ ...blogForm, title: e.target.value })}
                  className={`w-full border rounded-2xl px-4 py-3 text-xs font-medium focus:outline-none focus:border-[#E60000] ${isDarkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-slate-50 border-slate-200 text-slate-900"}`}
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className={`w-full border rounded-2xl px-4 py-3 text-xs font-bold focus:outline-none focus:border-[#E60000] ${isDarkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-slate-50 border-slate-200 text-slate-900"}`}
                  >
                    <option value="TikTok Live">TikTok Live</option>
                    <option value="UK Banking">UK Banking</option>
                    <option value="US Verification">US Verification</option>
                    <option value="SIM Guide">SIM Guide</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. Vodafone SIM Desk"
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full border rounded-2xl p-3 text-xs font-medium focus:outline-none focus:border-[#E60000] ${isDarkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-slate-50 border-slate-200 text-slate-900"}`}
                />
              </div>

              <div>
                <label className="block text-[11px] font-extrabold uppercase tracking-wider mb-1.5 text-slate-400">
                  Article Content (Supports Markdown headers ##, bullet points -, bold text **)
                </label>
                <textarea
                  rows={8}
                  required
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. TikTok Live Pakistan, Vodafone SIM, OTP Verification"
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200/40">
                <button
                  type="button"
                  onClick={() => setShowBlogModal(false)}
                  className="px-6 py-3 rounded-full border border-slate-300 dark:border-slate-700 text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="bg-[#E60000] hover:bg-[#CC0000] text-white text-xs font-extrabold px-8 py-3 rounded-full transition-all shadow-lg shadow-red-600/30 uppercase tracking-wider cursor-pointer"
                >
                  🚀 Publish Article
                </button>
              </div>
